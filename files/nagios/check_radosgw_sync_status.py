...
def check_ceph_status(args):
    """
    Used to check the status of multisite RadosGW Ceph. Uses the output of
    ...
    :raises: UnknownError, WarnError, CriticalError
    """
    ...
    if args.status_file:
        ...
        try:
            ...
            raise UnknownError(
                "UNKNOWN: radosgw-admin sync status command failed with error: {}".format(e))

    # If the realm name is empty, i.e. the first line is
    #    realm <some-uuid> ()
    # then we assume this means this is not multisite, so exit early.
    lines = status_data.split('\n')
    if len(lines) >= 1 and re.match(r"realm .* \(\)", lines[0].strip()):
        return "No multisite detected" 
    ...
    # Handle checks to do with given expected sites that should be connected.
    if args.sites:
        for site in args.sites.split(','):
            if re.search(r"data sync source:.*\(" + site + r"\)", status_data) is None:
                msgs.append("CRITICAL: Missing expected sync source '{}'".format(site))
    ...
    elif any(msg.startswith("WARNING") for msg in msgs):
        raise WarnError(complete_output)
    elif len(msgs) >= 1:
...
def parse_args(args):
    ...
    parser.add_argument('--sites', dest='sites',
                        default=None,
                        help="Check if the given sites, as a comma-separated "
                             "list, are present in the output. If they are missing "
                             "report critical.")
